...
- Each taxonomy **type** (Lebensmittel, Ausrüstung, Tiernahrung, …) has a **Lebensmittel-Typ** checkbox (`type.isFoodType`). Only categories under a food-tagged type can offer the Kalorien/Diversität choice at all — a Werkzeug or Tiernahrung category never shows it, it's simply always manual. This is a boolean tag rather than a name match ("is this type literally called Lebensmittel"), since a taxonomy can have more than one food type, and Tiernahrung is food but not *human* food — it stays untagged even though "food" describes it in the everyday sense. A food-tagged type gets a light green tint in the Taxonomie editor so it's visually distinct from Ausrüstung/Tiernahrung/etc. Unchecking a type's food tag never deletes any of its categories' kcal/macro/diversity data, only stops the calculator from using it — re-checking brings it straight back, same principle as the mode switch below.
...
- **Water category assignment** (`/config/planning.waterCategoryId`) — which taxonomy category receives the computed water target. Assigned here (not a Taxonomie planning mode) since it's a single admin-picked category, not a tagged set; the picker only offers categories under a food-tagged type, same gate as Kalorien/Diversität.
...
- **Subcategories under any computed category** (Kalorien, Diversität, or Wasser): the parent's computed total splits again across its subcategories with the same ±5% stepper mechanism, shown grouped by parent category name in the Unterkategorien section — this is the level a future shopping list can use directly, since it's where individual products' stock naturally rolls up.
...
**Status: Totals/Gaps view built, Lebensmittel only (Alerts and Shopping List not yet built).** Übersicht replaces the plain type/category tiles with tap-to-expand category cards (current vs. target, a progress bar, a color-coded reicht-X-Wochen chip) and a hero card for the Lebensmittel-wide total, all through the same kg/kg-per-Person&Tag/kcal/kcal-per-Person&Tag toggle Ziele uses. Sonstiges categories are excluded — they have no autonomy-duration/kcal framing. Current stock ("Vorrat") is computed from `/stockItems` via each batch's product → `subcategoryId` (never by the batch's own denormalized category/subcategory name text) using the `content`-parsing rule from Section 5; **Stück-tracked products inside a food category have no kg conversion factor and are excluded from every sum for now** — a known gap to close later. Tapping a subcategory (admin only) jumps to Settings → Bestandsliste pre-filtered to it via a new `subcategoryId`-based filter (not the free-text search).
