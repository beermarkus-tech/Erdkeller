...
- **Every product visible, including zero-stock/orphaned ones (Build 96)**: the table used to only ever show products that already had at least one real batch — a product with no stock, or one whose `subcategoryId` no longer resolves (see Build 95's orphan warning in Section 7), simply never appeared anywhere, which is exactly the gap that made a stray Wasser product in Ziele hard to actually clean up. Products with zero real batches now get a synthetic "phantom" row (`phantomBatchFor`/`allRows()` in `js/stock-table.js`, discriminated from a real batch by `id === null`) merged into the same list, muted (`.no-stock`, reduced opacity) with "Kein Bestand" instead of quantity/content/best-before, and its type/category/subcategory resolved live from Taxonomie (showing "⚠️ Unterkategorie fehlt" for a genuinely orphaned reference rather than a blank row). Tapping a phantom row opens a reduced edit view — rename only (the batch-specific fields and their date picker are hidden, with a note explaining there's no stock yet) — plus a new **"Produkt löschen"** button, the first place anywhere in the app that can delete a product from the catalog outright (`deleteDoc` on `/products/{id}`, already covered by the existing admin-only rule, no rules change needed). This is the direct fix for the stray/orphaned-product cleanup gap identified after Build 95.
