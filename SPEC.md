...
- **Crisis checklists**: fully open — admin can create new crisis types freely (e.g. "Power outage", "Water outage", "Medical emergency", "Evacuation"), each with its own first-steps checklist. Presented as a **large-text, high-contrast scrolling list** — a pure read-through reference, no checkboxes or step-tracking, since the goal is fast calm reading under stress rather than progress tracking. Adding, renaming, or deleting a crisis type re-renders both the editor and the live Krise card list immediately (Build 82 fix — the add/rename/delete handlers only re-rendered the editor, so a newly created or renamed type wouldn't show on the live list until an unrelated full refresh happened to fire).
