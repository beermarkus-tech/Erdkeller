...
**"+ Notiz" is a floating action button** (Build 108 — fixed bottom-right, signature green, "+" only, replacing the old inline dashed-border button) rather than a row in the flow, positioned above `nav.bottomnav` on mobile (which hides itself on tablet, `min-width:900px`, where the FAB just sits in the corner instead).
