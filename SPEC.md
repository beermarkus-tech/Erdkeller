...
**"+ Notiz" is a floating action button** (Build 108 — fixed bottom-right, signature green, "+" only, replacing the old inline dashed-border button) rather than a row in the flow, positioned above `nav.bottomnav` on mobile (which hides itself on tablet, `min-width:900px`, where the FAB just sits in the corner instead). Build 114 generalized this into a shared `.fab` class (was `.notes-fab`) and gave Kontakte's own "+ Kontakt" the identical treatment — the old inline text button is gone, replaced by the same round "+" in the same corner — and nudged both further from the corner (right 20→28px mobile / 24→32px tablet, bottom raised 16px mobile / 12px tablet) after it sat too close to the edge.
