...
- **Hardware/gesture back button (Build 77, `js/back-nav.js`)**: on Android, the back button/gesture drives the same "go back" action each screen's own on-screen control already performs — closing a Settings sub-panel, stepping back through the Stock guided flow, exiting Checklisten's edit mode, closing the crisis reference overlay — rather than doing nothing or exiting the app. Implemented as one sentinel browser-history entry that's pushed whenever any of those four nested states is entered and collapsed (via `history.back()`) whenever one closes any other way (an on-screen button, or switching bottom-nav tabs away from a screen left mid-state), so it never goes stale regardless of how the user actually navigates. Deliberately scoped to nested/modal states only — switching bottom-nav tabs is peer navigation, not depth, and isn't unwound on back (not how Android bottom-nav apps normally behave).
