...
The **edit screen** (`#note-edit-modal`, Build 103) is deliberately Google Keep-shaped rather than a form: a top bar with just a back button (left) and a photo button (right, with a small ✕ badge that appears once a photo is set — the button itself always opens the file picker to add or directly replace, confirmed with Markus there's no separate "replace vs. remove" prompt needed), a large borderless title input with no "Titel" label, the formatting toolbar, then a text box that flexes to fill the rest of the screen down to the bottom edge (or the on-screen keyboard). The photo is never shown while editing — only the top-bar button hints at it. **There is no Speichern button**: every change autosaves (`persistNow` in `js/notes.js`) — title/body debounced ~700ms after the last keystroke, a photo add/remove or toolbar click immediately — so the back button just closes an already-saved note. A brand new note that's never typed into leaves no Firestore doc behind; a note edited down to fully empty gets deleted the same way, rather than leaving a blank card in the grid. If a save fails (e.g. offline), the screen stays open with an inline error instead of closing over the unsaved change and losing it. `erdkeller:refresh` (the app-wide "reload your own data" broadcast most other screens listen for) only fires once on close, not on every autosave tick, so a stream of keystrokes doesn't trigger every other screen's Firestore re-fetch in the app.
