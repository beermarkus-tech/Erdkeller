...
Notizen (`js/notes.js`, Build 92, card grid + rich text Build 102, Keep-style edit screen Build 103, autosave reliability + keyboard fix Build 104, Duplizieren + double-tap Build 105, scroll fixes + list view Build 106, width-overflow fix Build 107): a **card grid** (or a dense **list view** — Build 106, see below) rather than a plain text-row list — one column on mobile, four across on tablet (`min-width:900px`) in card mode — with each note's photo as the card's hero image, title and a plain-text preview underneath. A note with **no photo shows a faint 📷 placeholder** in the same hero box instead of skipping it (Build 106), so every card is the same size whether or not it has a photo. Admins get three icons on the card itself: edit (pencil), **Duplizieren** (🗐, Build 105 — clones the note straight from the grid, no confirmation needed since an unwanted copy is as easy to delete as it was to create; the title gets a "(Kopie)" suffix when there is one), and delete (trash). **Deleting a note only ever happens from here**, neither screen below has its own delete control. A **search bar** above the grid matches on title or the body's plain-text content (tags stripped first, so formatting never affects matching).
