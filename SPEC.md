...
**Status: fully built** — Totals/Gaps, Alerts, and Einkaufsliste (shopping list), Lebensmittel + Wasser. Übersicht replaces the plain type/category tiles with tap-to-expand category cards (current vs. target, a progress bar, a color-coded reicht-X-Wochen chip) and a hero card for the Lebensmittel-wide total, all through the same kg/kg-per-Person&Tag/kcal/kcal-per-Person&Tag toggle Ziele uses. Wasser gets its own hero card next to the Lebensmittel one instead — one global current-vs-target figure in liters, no category cards, since a Wasser type has no per-category target to expand (see Section 7). Sonstiges categories are excluded entirely from the totals/gaps view — no autonomy-duration/kcal framing applies to them. Alerts and Einkaufsliste both follow a "preview strip/card + a modal for the full list" shape rather than navigating to a whole new screen. Alerts is a small horizontal-scroll strip of what's due within 3 months (danger-tinted if overdue/due this month, warn-tinted if due next month), "Alle anzeigen" opening a modal with a 1 Monat/6 Monate/Bis Jahresende horizon selector and the full sorted list. Unlike the rest of this screen, Alerts covers *every* batch regardless of Lebensmittel/Wasser/Sonstiges — a best-before date matters for medicine or batteries too, and there's no kcal/autonomy-framing dependency here forcing the exclusion. `bestBefore` is month-precision only ("MM/YYYY"), so the horizon comparison is a month-index comparison, not a day-level one. Tapping an alert jumps to Bestandsliste with the product name in the free-text search (no subcategoryId filter needed for a single product). Einkaufsliste (below the category cards) is the same gaps data re-surfaced as "what to buy": every subcategory below target (from the Kategorien cards, so Lebensmittel-scoped by construction), every individual product with a manual Ziele target that's short (deliberately *not* Lebensmittel/Wasser-scoped — a Sonstiges product target like "10 Batterien" belongs on a shopping list just as much as food does), and the Wasser hero's own shortfall if any. No checkboxes — a "mark as bought" tick would be state the app has to maintain for no real benefit, since actually checking stock back in via Einlagern is what closes the gap and drops the item off the list on its own. Current stock ("Vorrat") is computed from `/stockItems` via each batch's product → `subcategoryId` (never by the batch's own denormalized category/subcategory name text) using the `content`-parsing rule from Section 5; **Stück-tracked products inside a food category have no kg conversion factor and are excluded from every Kategorien/Wasser sum for now** — a known gap to close later. Einkaufsliste's product-level gaps sidestep this: a piece count needs no content-string parsing, so Stück-tracked products work there today even though they don't in the Kategorien cards. Tapping a subcategory (admin only) jumps to Settings → Bestandsliste pre-filtered to it via a new `subcategoryId`-based filter (not the free-text search).
