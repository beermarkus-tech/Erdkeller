...
**Status: sub-steps 1, 2, and 3 built and confirmed on real devices.** Cold offline start renders real data in well under a second on tablet and ~3s on phone (see Section 13.8 for the three-part cold-start-latency fix this took). Checklists split into `/checklists`+`/checklistItems` and cut over in Build 175 (Section 4, Section 13.4) — `js/checklists.js` now reads/writes the two flat collections live via `onSnapshot`, structural edits are enforced admin-only server-side for the first time, and the legacy `/config/checklists` document is frozen pending its own cleanup step. **Sub-steps 4–5** (`updatedAt` backfill on the remaining collections, crisis pre-cache confirmation) **not yet built.**
