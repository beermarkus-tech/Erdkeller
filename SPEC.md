...
- Admin-only: edit checklist name/recipients, add/edit/delete items and their frequency — **inline, not in Settings**: an admin-only pencil icon top-right of the Checklisten screen swaps the active tab (Wartung or Krise) between its normal view and the same editor markup in place; tapping it again ("✓ Fertig") swaps back. No standalone Settings → Checklisten submenu — same "edit where the content lives" reasoning as Contacts/Notes below, applied a build later once the pattern proved itself here first.
...
**Status: built**, with four deliberate deviations from the plan above, confirmed with Markus during Step 12 design — see Section 8 and Section 15 for the reasoning behind each: no Bestand/Ziele link at all (not even optional); no `nextDue` roll-forward, replaced by the period-bucket derived-completion model; no "einmalig" frequency (folded into yearly); no Settings → Checklisten submenu — editing is an admin-only pencil toggle inline on the Checklisten screen itself, added right after first shipping the submenu version, once it was clear the extra navigation hop wasn't earning its keep. Maintenance is grouped by checklist topic rather than by frequency, with a Fällig/Alle toggle. A one-time **"Bekannte Checkliste importieren"** button inside the inline maintenance editor seeds Markus's real ~113-item list (transcribed from his existing tracking sheet) into `/config/checklists` on first use.
*Test:* Tick a maintenance item, confirm it shows done and reappears unchecked once the calendar rolls into its next period (spot-check by editing `lastCompletedAt` in Firestore for a fast period, e.g. weekly); Fällig/Alle toggle filters correctly; as admin, tap the pencil icon top-right, confirm the view swaps to the editor in place, add/rename/delete a checklist and an item, tap "Fertig" and confirm it swaps back with the main list reflecting the change; confirm the pencil icon is invisible when signed in as a non-admin member; add a crisis type with steps via the same pencil toggle on the Krise tab, confirm it opens the full-screen high-contrast reference view with no checkboxes; run the import button once and confirm all groups and items appear, re-running it and confirming (per its warning) it duplicates rather than overwrites.
