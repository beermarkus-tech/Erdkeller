...
Notizen (`js/notes.js`, Build 92, redesigned Build 102): a **card grid** rather than a text-row list — one column on mobile, four across on tablet (`min-width:900px`) — with each note's photo, when it has one, as the card's hero image, title and a plain-text preview underneath. Tapping a card opens a detail modal (**full-screen on mobile, a large centered dialog on tablet**, scoped to `#note-detail-modal`'s own id so no other modal in the app is affected) with an explicit close (✕) button in the corner on top of the existing "tap outside to dismiss"; admin-only inline add/edit, same as Kontakte. A **search bar** above the grid matches on title or the body's plain-text content (tags stripped first, so formatting never affects matching).
