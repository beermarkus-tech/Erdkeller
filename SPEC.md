...
The **edit screen keeps the app's own beige page tint** (`var(--paper)`) instead of the generic `.modal-sheet`'s white default (Build 109 — scoped to `#note-edit-modal` only, the view screen stays white) — reads as a page within the app rather than a dialog floating over it; the title input and toolbar sit directly on that tint, while the text box itself (`.richtext-editor`) stays white, the same "white card on the beige page" look used everywhere else in the app.
