...
**Status: core offline path built and confirmed working on real devices (Builds 159–171), and checklist conflict resolution (13.4) landed in Build 175.** App-shell caching (13.3), Firestore persistence (13.2), cached-identity boot rendering, and the split of `/config/checklists` into per-item documents are all in place; the cold-start latency problem that made this feel broken for several builds is understood and fixed — see 13.8. **Not yet built**: the Funkstille/Verbindung settings panel (13.5), and backup hardening (Section 14's "Planned additions"). As of Build 157, before any of this, the app had *no* offline capability whatsoever — `service-worker.js` was a registration-only stub whose fetch handler passed straight through to the network, Firestore ran with its default memory-only cache, and the Firebase SDK itself was fetched from `gstatic.com` at runtime, with the app not starting at all offline. Section 2 lists offline-first as a core requirement; this section is both the architecture and, in 13.8, the record of what it actually took to make that true on real hardware.
...
**Status: sub-steps 1–4 built and confirmed on real devices.** Cold offline start renders real data in well under a second on tablet and ~3s on phone (see Section 13.8 for the three-part cold-start-latency fix this took). Checklists split into `/checklists`+`/checklistItems` and cut over in Build 175 (Section 4, Section 13.4) — `js/checklists.js` now reads/writes the two flat collections live via `onSnapshot`, structural edits are enforced admin-only server-side for the first time, and the legacy `/config/checklists` document is frozen pending its own cleanup step. `updatedAt`/`createdAt` backfilled onto every remaining collection and `/config/*` singleton in Build 177 (Section 20.2.4), the prerequisite for any future stale-write reconciliation. **Sub-step 5** (crisis pre-cache confirmation) **not yet built.**
...
- **Checklist conflict resolution — code shipped (Build 175), real two-device offline test still pending.** `/config/checklists` is split into `/checklists`+`/checklistItems` (Section 4, Section 13.4) and `js/checklists.js` cut over to reading/writing them live, but the actual acceptance test — both devices offline, ticking different items on different devices, reconnecting in each order, confirming last-write-wins resolves per-document rather than one device's edits vanishing wholesale — hasn't been run on the real phone and tablet yet. Treat this as unverified until that test passes.
