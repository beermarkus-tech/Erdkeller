...
- **Filters**: Type, Category, Subcategory (Build 96, chips — see below), Storage (chips), plus free-text search
...
- **Zero-stock products visible only when targeted (Build 96, narrowed Build 97)**: Build 96 first made every product with zero real batches show as a synthetic "phantom" row (`phantomBatchFor`/`allRows()` in `js/stock-table.js`, discriminated from a real batch by `id === null`) — but that meant a product simply run out via a normal Entnehmen lingered in the list too, which reads as broken rather than helpful. Build 97 narrows this: `allRows()` now only builds a phantom row for a product that still has an active Ziele product target (`targets.products[productId]`, loaded alongside the rest of this panel's config) — a plain "ran out, no target" product now just disappears, as expected. A targeted zero-stock row is muted (`.no-stock`, reduced opacity) and shows **"Kein Bestand · Ziel: …"** with the actual target amount and unit inline (`targetLabel`), not just a bare "no stock" label, so it's clear both that it's empty and what's needed. Type/category/subcategory are still resolved live from Taxonomie (showing "⚠️ Unterkategorie fehlt" for a genuinely orphaned reference). This also still covers the original stray/orphaned-product case (the ghost Wasser entry) without any special-casing, since a product only ever reaches Ziele's Produktziele list — and so gets flagged here — by having a target set in the first place. Tapping a phantom row opens a reduced edit view — rename only (the batch-specific fields and their date picker are hidden, with a note explaining there's no stock yet) — plus a **"Produkt löschen"** button, the first place anywhere in the app that can delete a product from the catalog outright (`deleteDoc` on `/products/{id}`, already covered by the existing admin-only rule). Deleting a targeted product also strips its now-dangling `targets.products` entry (same shape as Ziele's own "Aus"/`clearBtn` target-clear path) so no stale reference is left in `/config/targets`.
