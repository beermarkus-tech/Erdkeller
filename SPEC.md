...
**Status: sub-steps 1–2 built and confirmed on real devices — cold offline start renders real data in well under a second on tablet and ~3s on phone (see Section 13.8 for the three-part cold-start-latency fix this took). Sub-steps 3–5 (checklist per-item split, `updatedAt` backfill, crisis pre-cache) not yet built.**
