...
- **Product-level target overrides** stay available exactly as before, independent of everything above — e.g. "I specifically want 5 kg of this one product," or the vitamin C tablet calculation above. New products can be created directly from the Produktziele picker (name, unit, Unterkategorie) instead of needing the guided Bestand flow first — Sonstiges' own version of this flow is the open-unit modal described above. **The Produktziele picker's own product list wasn't scrollable** once the catalog grew tall enough (Build 113) — traced to the shared `.modal-sheet` base rule (used by most plain modals in the app: this picker, contact detail, table edit, date pickers, …) having no `max-height`/`overflow-y` at all, so a sheet taller than the viewport just extended past `.modal-overlay`'s fixed box with literally no scroll mechanism anywhere in the chain, pushing top content off-screen with no way to reach it. This is a different bug class from the `overflow:hidden`-blocks-touch-scroll one documented elsewhere (Notizen, above) — here there was no scroll container at all, not a blocked one. Fixed generically at `.modal-sheet` (`max-height:85vh; overflow-y:auto;`) rather than scoped to this one modal, since the same gap could bite any sufficiently tall plain modal; confirmed no conflict with Notizen's view/edit screens, which already set their own height/overflow-y via a more specific selector.
...
- Presented as a **sortable, filterable table** rather than a fixed grouped list — every row still shows Product, Subcategory, Category, Type, Quantity, Content, Best-before, Storage, but only **Product, Quantity, Content, Best-before** are tap-to-sort badges (Build 100 — Subcategory/Category/Type/Storage dropped from the sort badges since they already have their own dedicated filter chips just below)
